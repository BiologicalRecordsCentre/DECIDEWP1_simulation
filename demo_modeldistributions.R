...
#loop over all 10 species - too slow on laptop, bit clunky
...
for (j in 1:length(pa_sets)){
  ...
  for (j in 1:length(pa_sets)){
    ...
    par(mfrow=c(3,2))
    par(mar = c(2,2,2,2))
    plot(virt_comm1[[j]], main = "Environmental suitability")
    plot(pa[[j]]$probability.of.occurrence, main = "Probability of occurrence")
    plot(pa[[j]]$pa.raster, main = "Presence absence")
    points(sp.obs[[j]]$sample.points[is.na(sp.obs[[j]]$sample.points$Observed),1:2], pch = 20)
