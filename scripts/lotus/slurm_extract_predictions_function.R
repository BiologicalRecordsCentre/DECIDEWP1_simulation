...
extract_predictions <- function(community_folder, community_version, AS_version, model, method){
  ...
  for (j in 1:length(species_list)){
    ...
    # save model averages
    write.csv(method_mod_av, file = paste0(community_folder, 'preds_and_obsvs/', species, '_', AS_version, '_', community_name, "_model_averages.csv"))
    
    # save observations file
    write.csv(method_obsvs, file = paste0(community_folder, 'preds_and_obsvs/', species, '_', AS_version, '_', community_name, "_observations.csv"))
