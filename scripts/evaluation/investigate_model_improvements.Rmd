...
ggplot(na.omit(subset(init_new_locs, uptake!=0)), aes(x = n, y = method, fill = method, colour = method)) +
  geom_density_ridges(stat = "binline", alpha = 0.5) +
  facet_wrap(~uptake) +
  theme_bw() +
...
ggplot(na.omit(subset(etp, uptake != 0 & method != 'initial')), 
  ...
  geom_bar(position="fill") +
  scale_fill_discrete(name = 'Percentage change\nin AUC') +
...
nmods_c <- (cno1+cno2+cno3) / (msen + aucn + corrn) + plot_layout(guides = 'collect')  +
