...
init <- data.frame(do.call(rbind, replicate(6, as.matrix(elp[elp$method=='initial',]), simplify=FALSE)))

init$method <- rep(unique(elp$method[elp$method!='initial']), 
                   each = nrow((elp[elp$method=='initial',])))

elpdf <- rbind(elp, init) %>% 
  mutate(x = as.numeric(xvar),
         y = as.numeric(yvar))

ggplot(elpdf[elpdf$method!='initial',], aes(x,y,col = factor(asv))) +
  geom_path(size = 0.7) +
  xlab('PC1') + ylab('PC2') +
  facet_wrap(~method) +
  theme_bw()
